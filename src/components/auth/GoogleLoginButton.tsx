...
const GoogleLoginButton: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!retailerDoc.exists() && !isAdmin) {
        // First time Google login for non-admins → create pending retailer entry
  ...
  return (
    <button
      ...
      disabled={loading}
      className="w-full h-14 bg-white border border-slate-200 rounded-2xl flex items-center justify-center gap-3 hover:bg-slate-50 hover:shadow-lg transition-all duration-300 transform active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed group"
    >
      {loading ? (
